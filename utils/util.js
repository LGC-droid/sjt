...
/*
  *存储用户当前定位位置
  */
 function set_user_ad_info(info,uid) { //初始化加载 
  var that = this;  
  basePost(U({
    c: 'Public_api',
    a: 'set_user_ad_info',
    ...
    city_code:info.ad_info.city_code,
    nation_code: info.ad_info.nation_code,
    lat: info.location.lat,
    lng:  info.location.lng,
    name: info.ad_info.name,
    nation:info.ad_info.nation,
    province:info.ad_info.province,
    city: info.ad_info.city,
    district:info.ad_info.district,
    address:info.address,
    recommend_address:info.formatted_addresses.recommend,
    uid:uid
  }, function (res) {
...
module.exports = {
  ...
  currentPagePath,
  previousPageClass, 
  set_user_ad_info:set_user_ad_info
}
